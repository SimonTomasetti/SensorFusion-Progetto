...
#define KALMAN_NAME geolocalization
#define KALMAN_NUM_STATES 6
#define KALMAN_NUM_INPUTS 6

// create the measurement structure
#define KALMAN_MEASUREMENT_NAME position
#define KALMAN_NUM_MEASUREMENTS 6
    ...
    matrix_data_t *aux, matrix_data_t *predictedX, matrix_data_t *temp_P, matrix_data_t *temp_BQ)
{
    ...
    matrix_init(&kf->x, num_states, num_states, x);
    ...
    matrix_init(&kf->temporary.predicted_x, num_states, num_states, predictedX);
...
*/
void kalman_measurement_initialize(    kalman_measurement_t *kfm,
